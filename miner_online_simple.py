...
import requests
# Optional at import-time: allow GUI to import this module without pymongo installed
try:
    ...
    class PyMongoError(Exception):  # type: ignore
        pass
    class ServerSelectionTimeoutError(Exception):  # type: ignore
        pass
    HAVE_PYMONGO = False
try:
...
_GEOIP_READER: Optional["GeoIP2Reader"] = None
...
def _compose_hardware_doc(
    ...
    *,
    ...
    doc["mac"] = {
        "miner_mac": mac.get("miner_mac") or "",
        "mac_registered": mac.get("mac_registered") or "",
...
def redact_mongo_uri(uri: str) -> str:
    """Redact credentials in a MongoDB URI for logging.
    Keeps hosts and database visible; masks any username/password.
    """
    try:
...
def load_config() -> Dict[str, Any]:
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            cfg = json.load(f)
        if isinstance(cfg, dict) and "mongo_uri" in cfg:
            return cfg
    except Exception:
        pass
    try:
...
def connect_mongo(uri: str, tlsCAFile: Optional[str] = None) -> MongoClient:
    if not HAVE_PYMONGO:
        raise RuntimeError("pymongo not available in this build; DB operations are disabled")
    opts = dict(serverSelectionTimeoutMS=8000, retryWrites=True, appname="FRY-Miner")
    if tlsCAFile:
        if not os.path.isabs(tlsCAFile):
            tlsCAFile = os.path.join(app_dir(), tlsCAFile)
        opts["tlsCAFile"] = tlsCAFile
    client = MongoClient(uri, **opts)
    client.admin.command("ping")
    return client

def ensure_device_is_installed_flag(client: MongoClient, miner_key: str):
...
def ensure_device_installed_version(client: MongoClient, miner_key: str, version: str):
...
def other_active_installation_exists(client: MongoClient, miner_key: str, install_id: str, window_seconds: int = 30) -> bool:
...
def acquire_installation_lease(client: MongoClient, miner_key: str, install_id: str, lease_seconds: int = 900) -> bool:
...
def renew_installation_lease(client: MongoClient, miner_key: str, install_id: str, lease_seconds: int = 900) -> bool:
...
def _h3_to_country(h3_index: Optional[str], area_threshold: float = COUNTRY_AREA_THRESHOLD) -> Tuple[Optional[str], Optional[float], bool]:
    if not (h3_index and HAVE_H3 and HAVE_SHAPELY):
        return None, None, False
    try:
...
def registered_hexid_from_devices(client: MongoClient, miner_key: str) -> Optional[str]:
...
def write_location_daily(coll, client: MongoClient, miner_key: str, ts: dt.datetime):
...
def registered_mac_from_devices(client: MongoClient, miner_key: str) -> Optional[str]:
...
def main():
    ...
    try:
        ...
        log.info(
            "Starting service v%s | ProgramData=%s | config=%s | mongo_uri=%s | tlsCAFile=%s | interval=%s | lease_seconds=%s",
            VERSION,
            data_dir(),
            cfg_src,
    ...
    while client is None:
        try:
            ...
            log.error("Initial Mongo connect failed: %s", e)
            time.sleep(5)
